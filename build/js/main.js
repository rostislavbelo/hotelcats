...
(function () {
  ...
  const inputActive = document.querySelector('.popup_booking-input-name-owner input');
...
(function () {
  ...
  const bullet = roomSection.querySelectorAll('.slider__bullet');
  const bullet1 = roomSection.querySelector('.slider__bullet-1');
  const bullet2 = roomSection.querySelector('.slider__bullet-2');
  const bullet3 = roomSection.querySelector('.slider__bullet-3');
  const buttonBack = roomSection.querySelector('.slider__button-back');
  const buttonNext = roomSection.querySelector('.slider__button-next');

  if (roomSection, room, room1, room2, room3, bullet1, bullet2, bullet3) {
    ...
    const selectionsSliderBullet = (roomArg, bulletArg) => {
      bulletArg.addEventListener('click', () => {
        ...
        bulletArg.classList.add('slider__bullet--active');
      });
    };
    ...
    buttonNext.addEventListener('click', () => {
      if (room1.classList.contains('rooms__item--active')) {
        ...
        bullet1.classList.remove('slider__bullet--active');
        bullet2.classList.add('slider__bullet--active');
      } else {
        if (room2.classList.contains('rooms__item--active')) {
          ...
          bullet2.classList.remove('slider__bullet--active');
          bullet3.classList.add('slider__bullet--active');
        } else {
          if (room3.classList.contains('rooms__item--active')) {
            ...
            bullet3.classList.remove('slider__bullet--active');
            bullet1.classList.add('slider__bullet--active');
          }
        }
      }
    });
...
(function () {
  const blockSliderButtons = document.querySelectorAll('.slider__buttons');
  const commentsSection = document.querySelector('#comments');
  const sliderList = commentsSection.querySelector('.comments__list');
  const bullet = commentsSection.querySelectorAll('.slider__bullet');
  const bullet1 = commentsSection.querySelector('.slider__bullet-1');
  const bullet2 = commentsSection.querySelector('.slider__bullet-2');
  const bullet3 = commentsSection.querySelector('.slider__bullet-3');
  const buttonBack = commentsSection.querySelector('.slider__button-back');
  const buttonNext = commentsSection.querySelector('.slider__button-next');
  let offset = 0;

  if (blockSliderButtons, commentsSection, bullet, bullet1, bullet2, bullet3, buttonBack, buttonNext) {
    blockSliderButtons.forEach((block) => {
      block.classList.add('slider__buttons--active');
    });
    ...
    const setImageBullet = (offsetValue, position1, position2, position3, position4, position5, position6) => {
      ...
      position2.classList.add('slider__bullet--active');
      position3.style.pointerEvents = 'auto';
      position4.style.pointerEvents = 'auto';
      position5.classList.remove('slider__bullet--active');
      position6.classList.remove('slider__bullet--active');
    };
